...
for s in signal:
    ...
    for syst in systematics:
        ...
        if len(obsBins) < 1:
            # Uniform binning
            uniformBinning = True
            uniformBinWidth = morph[masses[0]].GetBinLowEdge(2) - morph[masses[0]].GetBinLowEdge(1)
            obsBins = []
        ...
        for m in masses:
            for b in xrange(1, morph[m].GetNbinsX() + 1):
                if uniformBinning:
                    ...
                    h[s][syst].Fill(m / 10.0, morph[m].GetBinCenter(b), morph[m].GetBinContent(b) / morph[m].GetXaxis().GetBinWidth(b))
